...
> **Dated record (2026-03-30) — not maintained.** Paths, line numbers, counts
...
> **Gap:** 4.3 Save System Design
> **Status:** Spec complete, pending implementation
> **Date:** 2026-03-30

## Overview

Defines the save/load system for Pendulum of Despair: what gets
persisted, when saves happen, how slots work, how death interacts with
saves, and how saves survive game updates. Engine-agnostic — specifies
the contract, not the implementation.

**Design principle:** Save the player's *choices and progress*. Derive
everything that is a *function of those choices* at load time.

---

## 1. Persisted vs Derived State
...
**Why this split matters:** Most balance patches (stat tweaks, price
changes, rebalanced items) need zero save migration because the values
come from data tables, not the save file.

---

## 2. Configuration — Global, Not Per-Slot

Player preferences live in a separate global config file, completely
independent of save data:
...
Loading a save never changes settings. This is the FF6 model — config
is system-level. The config file persists alongside saves but is not
part of any save slot.
...
Array of up to 6 characters (all recruited party members, not just
active battle party).
...
**Note:** Status ailments persist in saves. The player may save while
poisoned. Resting with a consumable item clears ailments; saving
without resting does not.
...
Post-game completion tracking (4 categories per postgame.md). Displayed
at Pendulum tavern via Sable.
...
- During boss fights
- During boss rush tiers (save suppressed — see Section 9)
...
their accumulated progress is preserved. This is critical to the
anti-frustration design — a player who dies 5 times to a boss has
gained 5 runs worth of XP from trash fights, and that progress survives
across sessions.
...
resumes. Full HP/MP restore applies (standard level-up behavior).
...
- The final boss **cannot be re-fought** in the post-game world —
  ...
  and No NG+ decision. (Boss rush memorial encounters are separate —
  see Section 9.)

Loading a post-game save places the player in the post-epilogue world
with full access to optional content.

---

## 9. Boss Rush Save Suppression

During boss rush tiers (post-game content per postgame.md):
...
## 11. Storage Notes (Engine-Agnostic)

This spec does not prescribe a storage format or location. The
implementing engine should:
