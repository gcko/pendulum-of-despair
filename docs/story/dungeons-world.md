...
- **Act:** **II — this is the canonical assignment (#287).** Canonically the dungeon is unreachable in Act I: Duskfen opens only on `diplomatic_mission_start` ([events.md](events.md) flag 8, listed under Act II Flags, and again in the Act II "New Areas Opening" list), and flag 8 can only fire after `pendulum_to_capital` (flag 6), the event whose own entry reads "Act I ends. Act II begins." [outline.md](outline.md) § The Diplomatic Mission makes the same sequence explicit, listing "clear Fenmother's Hollow" as step 1 of three. Reachability settles it; nothing proposes a way in before the diplomatic mission. **The current build does not implement that gate** — the `FenmothersHollow` transition in `game/scenes/maps/overworld.tscn` carries no `metadata/required_flag` (unlike its `DuskfenShrineEntry` sibling), and `diplomatic_mission_start` is never set by any content, so the dungeon is enterable from the start. That is issue #193 (GAP-043), not a competing act assignment: the canon above is what the build should enforce, and #193 is the work of making it do so. Several docs nevertheless *tune* it as Act I content — the roster sits in `game/data/enemies/act_i.json` at Lv 6-12 with a Lv 12 boss, [bestiary/act-i.md](bestiary/act-i.md) hosts its stat table, and [economy.md](economy.md) § Act I counts the Fenmother's boss gold. That is a power-band statement, not an act assignment: the dungeon is the *first* content of Act II and is deliberately pitched at the ceiling of Act I's curve. Where a doc says "Act I" about this dungeon it is stale, not a competing reading.
- **Recommended Level:** 12-15 — the party arrives straight off the Act I/II transition, so this band is the top of the Act I curve rather than the middle of Act II's.
...
- **Grayscale shift:** The color palette desaturates 10% per section. By Section 4, the screen is nearly monochrome. Only ley-line save points retain color.
