...
- **Permanent and independent of level.** A capsule gain is banked separately from the leveled base stats. Leveling up recomputes base stats from the growth curve, and that recompute must never subtract the banked capsule gains — they are re-applied on top of the new base. The same holds for the narrative milestone stat spikes, in every act (progression.md § Narrative Milestone Stat Spikes).
- **Visible everywhere the stat is.** Capsule gains ride the same effective-stat formula the status/equip menus and combat read ([progression.md](progression.md) § Equipment and Buffs), exactly like an equipment bonus. That formula also produces max HP/MP, though no HP or MP capsule exists in the current catalog, so that term is zero today.
