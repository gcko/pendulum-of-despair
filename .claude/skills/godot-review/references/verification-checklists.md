...
- [ ] **Physics trigger gating during cutscenes:** For every `body_entered` / `area_entered` signal handler, verify the handler checks the cutscene state before processing. Cutscene `move` commands walk the player's CharacterBody2D, which triggers Area2D overlaps. `_unhandled_input` gating is NOT sufficient — physics signals bypass input handlers. Since GAP-087 the handlers are split: boss triggers stay on `exploration.gd:on_boss_trigger_entered` and read `_in_cutscene` directly, while dialogue and map-transition triggers moved to `exploration_interactions.gd` (`on_dialogue_trigger_entered`, `on_transition_body_entered`) and go through `_is_player_free_to_trigger`, which asks `_exploration.is_in_cutscene()`. Grepping only exploration.gd for `_in_cutscene` will miss the moved half. (PR #142: boss/transition/pitfall handlers lacked _in_cutscene guard)
...
- [ ] **Dead flag detection:** For every `metadata/flag` on transitions, verify the transition handler actually reads and sets that flag. `exploration_interactions.gd:on_transition_body_entered` (moved out of exploration.gd in GAP-087; exploration.gd:on_transition_body_entered is now a one-line forwarder) only checks `required_flag`, NOT `flag`. If a transition has `metadata/flag` set, it is dead metadata — either add flag handling to the transition handler or remove it. (PR #141: ironmouth_entered flag on overworld transition was never set)
