...
## Fixture stream sample rate and duration. A bare `AudioStreamWAV.new()` has no
## data, so its length is 0.0 and playback ENDS ~5ms after `play()` — measured on
## macOS with no frame boundary in between. That is short enough for a loaded CI
## runner to close the window between two consecutive GDScript statements, which
## is what made test_play_music_with_crossfade_starts_new_track flaky (#420).
## Every fixture here is therefore a stream of real duration: 4 seconds, against
## a suite that runs all of these tests in under one second, so an assertion
## about `playing` reports the audio system rather than the clock.
...
## Build a silent but genuinely 4-second-long stream (8-bit mono, all zeros).
...
func test_fixture_stream_has_real_length() -> void:
	# Guards the guard: every `playing` assertion below is only meaningful while
	# the fixture outlasts the test. A bare AudioStreamWAV.new() has length 0.0
	# and stops ~5ms after play(), which is what made #420 flaky. If this fails,
	# the fixture has regressed to zero-length and those assertions are racing.
...
func test_rapid_music_switch_keeps_newest_active() -> void:
	...
	var a: AudioStreamWAV = _fixture_stream()
	var b: AudioStreamWAV = _fixture_stream()
	var c: AudioStreamWAV = _fixture_stream()
	...
	assert_eq(_am._music_fade.stream, b, "Fade slot should hold the track it interrupted")
...
func test_play_music_crossfade_swaps_to_fade() -> void:
	var stream_a: AudioStreamWAV = _fixture_stream()
	var stream_b: AudioStreamWAV = _fixture_stream()
	_am._play_music_with_stream(stream_a, "track_a", 0.0)
	var first_player: AudioStreamPlayer = _am._music_active
	_am._play_music_with_stream(stream_b, "track_b", 0.0)
	assert_eq(_am._current_music, "track_b", "Current should be track_b")
	assert_eq(_am._music_fade, first_player, "Fade slot should be the player that held track_a")
...
func test_play_ambient_crossfade_swaps() -> void:
	...
	var first_player: AudioStreamPlayer = _am._ambient_active
	_am._play_ambient_with_stream(stream_b, "cave", 0.0)
	assert_eq(_am._current_ambient, "cave", "Current should be cave")
	assert_eq(_am._ambient_fade, first_player, "Fade slot should be the player that held forest")
...
func test_play_music_with_crossfade_starts_new_track() -> void:
	...
	_am._play_music_with_stream(stream_a, "track_a", 0.0)
	var first_player: AudioStreamPlayer = _am._music_active
	# Second track with crossfade > 0 must swap players: the new track takes over
	# the other player of the pair while the first player fades out. Assert that
	# swap directly rather than via `playing`, which is only incidental to it.
	...
	assert_eq(_am._music_active.stream, stream_b, "Active player should hold the new stream")
	assert_ne(
		_am._music_active, first_player, "Crossfade should move the new track off the old player"
	)
	assert_eq(
		_am._music_fade, first_player, "The player that held track_a should be the fading one"
	)
	assert_eq(_am._music_fade.stream, stream_a, "Fading player should still hold the old stream")
	# Meaningful now that the fixture is FIXTURE_SECONDS long: with a zero-length
	# stream this raced to false ~5ms after play() and failed CI once (#420).
